...
func TestStringToInstructions(t *testing.T) {
	inst := StringToInstructions(inputStr)
	assert.Equal(t, 4, len(inst), "they should be equal")
	assert.Equal(t, 10, inst[0].x, "they should be equal")
	assert.Equal(t, 12, inst[0].xp, "they should be equal")
	assert.Equal(t, 10, inst[0].y, "they should be equal")
	assert.Equal(t, 12, inst[0].yp, "they should be equal")
	assert.Equal(t, 10, inst[0].z, "they should be equal")
	assert.Equal(t, 12, inst[0].zp, "they should be equal")
	assert.Equal(t, true, inst[0].state, "they should be equal")
}
...
func TestCubesBig(t *testing.T) {
	inst := StringToInstructions(BigStr)
	cubesOn := Cubes(inst, 50)
